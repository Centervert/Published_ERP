...
export function EmailPromptForm({ onSubmit, isLoading }: EmailPromptFormProps) {
  ...
  const handleGenerateSubject = async (showToast = true) => {
    if (!description.trim()) {
      if (showToast) toast.error('Please describe your email first');
      return;
    }
    ...
    try {
      ...
      if (data?.subject) {
        ...
        if (showToast) toast.success('Subject generated!');
      }
    } catch (error) {
      console.error('Error generating subject:', error);
      if (showToast) toast.error('Failed to generate subject. Please try again.');
    } finally {
      setIsGeneratingSubject(false);
    }
  };

  // Auto-generate subject when content is complete and subject is empty
  const hasAutoGenerated = useRef(false);
  
  useEffect(() => {
    // Only auto-generate once when all required content is filled and subject is empty
    if (
      emailType && 
      ...
      handleGenerateSubject(false);
    }
  }, [emailType, description, tone]);

  // Reset auto-generate flag if subject is cleared manually
  useEffect(() => {
    if (!subject.trim()) {
      ...
      <div className="space-y-2 pt-4 border-t">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            onClick={() => handleGenerateSubject(true)}
